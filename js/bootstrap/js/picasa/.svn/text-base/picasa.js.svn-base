...
Picasa.prototype = {
	...
	// 参数
	loadingImage	: RootPath+'js/bootstrap/js/picasa/Picasa_Loading.gif',		// 加载中提示图片路径
	errorImage		: RootPath+'js/bootstrap/js/picasa/Picasa_Error.gif',		// 加载错误提示图片路径
	...
	// 初始化
	initialize : function() {
		this.arrItems=[];//因为不刷新网页，所以，每次初始化时，都把项目清空，否则，文件夹进入后，会累加之前的图片。
		var self = this;
		...
		for (var i=0; i<count; i++){
			...
			if (rel.match(/picasa/i)){
				info = decodeURIComponent(rel.replace(/picasa|[\[\s\]]/ig, '')) +',';
				...
				anchors[i].ondblclick = function(){self.display(this); this.blur(); return false;}
	...
	createFrame : function() {
		...
		this.frame.innerHTML =
			...
			'<div id="PV_Hint"></div>'+
			'<div id="PV_Control">'+
			'	<ul id="PV_Items" style="display:none"></ul>'+
			'	<div id="PV_Buttons">'+
			...
			'		<div id="PV_AutoPlay"><a href="javascript: void(0);" id="PV_Btn_AutoPlay">'+Language.AutoPlay+'</a></div>'+
			'	</div>'+
			'</div>';
		document.body.appendChild(this.frame);

		// 框架对象
		this.Picture	= myPic('PV_Picture');				// 大图
		this.Loading	= myPic('PV_Loading');				// 加载提示图
		this.Error		= myPic('PV_Error');				// 错误提示图
		this.fPerHint	= myPic('PV_PerHint');				// 缩放百分比提示框
		this.fHint		= myPic('PV_Hint');					// 底部提示框
		this.fControl	= myPic('PV_Control');				// 控制栏
		this.fItems		= myPic('PV_Items');				// 列表项
		this.fButtons	= myPic('PV_Buttons');				// 控制按钮

		// 按钮对象
		this.btnZoomIn		= myPic('PV_Btn_ZoomIn');		// 放大(+)
		this.btnZoomOut		= myPic('PV_Btn_ZoomOut');		// 缩小(-)
		this.btnZoomActual	= myPic('PV_Btn_ZoomActual');	// 原始尺寸(1:1)
		this.btnPrev		= myPic('PV_Btn_Prev');			// 上一张图片
		this.btnSlide		= myPic('PV_Btn_Slide');		// 幻灯片演示
		this.btnNext		= myPic('PV_Btn_Next');			// 下一张图片
		this.btnAutoPlay	= myPic('PV_Btn_AutoPlay');		// 自动播放
	...
	setElementEvent : function() {
		...
		this.btnPrev.onmouseover = function() {self.hint(self.lng.prev);myPic('PV_Select').className='PV_Btn_PrevOver';}
		this.btnPrev.onmouseout = function() {self.hint();myPic('PV_Select').className='PV_Btn_Normal';}
		...
		this.btnSlide.onmouseover = function() {self.hint(self.lng.slide);myPic('PV_Select').className='PV_Btn_SildeOver';}
		this.btnSlide.onmouseout = function() {self.hint();myPic('PV_Select').className='PV_Btn_Normal';}
		this.btnSlide.onclick = function() {return false;}
	
		this.btnNext.onmouseover = function() {self.hint(self.lng.next);myPic('PV_Select').className='PV_Btn_NextOver';}
		this.btnNext.onmouseout = function() {self.hint();myPic('PV_Select').className='PV_Btn_Normal';}
	...
	display : function() {
		...
		this.mask.style.display = 'none';
		$(this.mask).fadeIn(300);//当图片开始显示的时候，背景淡入效果
	},
	...
	loadedAction : function() {
		...
		this.frame.style.display = 'none';
		$(this.frame).fadeIn(300);//当图片加载成功后，显示时，淡入效果
	},
	loadError : function() {
		this.Picture.style.display = 'none';
		this.Loading.style.display = 'none';
	...
	close : function() {
		...
		displaySelect(true);
		$(this.mask).fadeOut(300);//关闭图片浏览器后，背景和当前图片淡出效果
