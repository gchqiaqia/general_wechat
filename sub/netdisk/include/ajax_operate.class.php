...
class Operate extends Bn_Basic {
		...
		if (is_dir ( $s_path )) {
			if ($dh = opendir ( $s_path )) {
				while ( ($file = readdir ( $dh )) != false ) {
					...
					$a_temp = array ('name' => iconv ( $this->getEncode (), 'utf-8', $file ), 'size' => $size, 'path' => iconv ( $this->getEncode (), 'utf-8', 'userdata/netdisk/' . md5 ( $n_uid . 'ctisss' ) . $s_refreshpath . $file ), 'date' => date ( "Y-m-d H:i:s", $fmt ), 'isfile' => $b_isfile );
					
					array_push ( $a_files, $a_temp );
				}
				closedir ( $dh );
			}
			$a_general = array ('path' => iconv ( $this->getEncode (), 'utf-8', $s_refreshpath ), 'success' => 1, 'text' => Text::Key ( 'OperationSuccess' ), 'files' => $a_files );
		} else {
			$a_general = array ('path' => iconv ( $this->getEncode (), 'utf-8', $s_refreshpath ), 'success' => 0, 'text' => Text::Key ( 'OperationError01' ), 'files' => '' );
		}
	...
	public function CreateFolder($n_uid) {
		...
		if (mkdir ( $s_path, 0777 )) {
			$a_general = array ('success' => 1, 'text' => Text::Key ( 'OperationSuccess' ) );
		} else {
			$a_general = array ('success' => 0, 'text' => Text::Key ( 'CreateFolderError' ) );
		}
		echo (json_encode ( $a_general ));
	}
	...
	public function Zip($n_uid) {
		...
		if (! is_writeable ( $basic_path )) {
			$a_general = array ('success' => 0, 'text' => Text::Key ( 'ZipError' ) );
		} else {
			$files = array ();
			for($i = 0; $i < $list_num; $i ++) {
	...
	public function VerifyDiskSpace($n_uid, $n_size) {
		...
		if ($n_free >= $n_size) {
			...
			$a_general = array ('success' => 0, 'text' => Text::Key ( 'DiskSpaceFull' ) );
			echo (json_encode ( $a_general ));
			exit ( 0 );
		}
	}
	public function GetDiskSpace($n_uid) {
		...
		$a_general = array ('success' => 1, 'total' => $o_user_info->getDiskSpace (), 'used' => $a_result ['size'] );
		echo (json_encode ( $a_general ));
	}
	public function PropertyFiles($n_uid) {
		...
		//构造结果
		$a_general = array ('success' => 1, 'type' => $s_type, 'filetype' => $s_filetype, 'data' => $a_data );
		echo (json_encode ( $a_general ));
	}
}
